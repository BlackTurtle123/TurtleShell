...
export function getAssetsId(tx): Array<string> {
    const assets = {};
    const feeAssetId = tx.matcherFee && tx.matcherFee.assetId ? tx.matcherFee.assetId : tx.feeAssetId || 'TN';
    const amountAssetId = tx.amount && tx.amount.assetId ? tx.amount.assetId : tx.amountAssetId || 'TN';
    const priceAssetId = tx.price && tx.price.assetId ? tx.price.assetId : tx.priceAssetId || 'TN';
...
export function getFee(tx) {
    return typeof tx.matcherFee === 'object' ? tx.matcherFee : { coins: tx.matcherFee, assetId: 'TN' };
}
...
export function getPrice(tx = null) {
    return typeof tx.price === 'object' ? tx.price : { coins: tx.price, assetId: 'TN' };
}
