...
async function setupBackgroundService() {
  ...
  const updateBadge = () => {
    ...
    const notifications = backgroundService.notificationsController.getGroupNotificationsByAccount(
      selectedAccount
    );
...
class BackgroundService extends EventEmitter {
  ...
  getState() {
    ...
    const { selectedAccount } = state;
    const myNotifications = this.notificationsController.getGroupNotificationsByAccount(
      selectedAccount
    );
  ...
  setupUiConnection(connectionStream) {
    ...
    const remoteHandler = remote => {
      ...
      //Microsoft Edge doesn't support browser.windows.close api. We emit notification, so window will close itself
      const closeEdgeNotificationWindow = remote.closeEdgeNotificationWindow.bind(
        remote
      );
